...
public class StatusView extends FrameLayout {
    ...
    private @LayoutRes
    int loginLayoutId = R.layout.sv_login_layout;
    ...
    public StatusView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            }else if (attr == R.styleable.StatusView_sv_login_view) {
                loginLayoutId = ta.getResourceId(attr, loginLayoutId);
            }
     ...
     * 设置自定义 Login 布局文件
     */
    public void setLoginView(@LayoutRes int loginLayoutRes) {
        this.loginLayoutId = loginLayoutRes;
    }
    ...
    /**
     * 设置 Login 布局首次显示时的回调，可在回调中更新布局、绑定事件等
     */
    public void setOnLoginViewConvertListener(StatusViewConvertListener listener) {
        listenerArray.put(loginLayoutId, listener);
    }
